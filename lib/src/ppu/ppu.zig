    ...
    fn spriteEval(self: *Self) void {
        ...
        if (self.cycle == 257) {
            ...
            for (0..num_sprites) |i| {
                ...
                const sprite_y: u8 = self.secondary_oam[j];
                ...
                const raw_tile = tile_index;

                var row: u8 = @truncate(self.scanline - sprite_y);

                // For 8x16: extract PT from tile bit 0, clear bit 0 for tile address
                if (tall_sprites) {
                    tile_index &= 0xFE;
                }
                ...
                // 8x16: use tile bit 0 for PT selection; 8x8: use PPUCTRL
                const pt_number: u1 = if (tall_sprites)
                    @truncate(raw_tile & 1)
                else
                    if (self.ppu_ctrl.pattern_sprite) 1 else 0;

                var pt_lo = self.fetchFromPatternTable(tile_index, true, pt_number, row);
                var pt_hi = self.fetchFromPatternTable(tile_index, false, pt_number, row);
